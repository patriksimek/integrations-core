...
def coverage_sources(check):
    ...
    if check == 'datadog_checks_base':
        ...
        return (
            '../{check}/datadog_checks/{check}'.format(check=check),
            '../{check}/tests'.format(check=check),
        )
...
def test(checks, bench, coverage, cov_missing, cov_keep, verbose):
    ...
    test_env_vars = {
        ...
        'DDEV_COV_MISSING': 'True' if cov_missing or testing_on_ci else 'False',
            ...
            with EnvVars(test_env_vars):
                ...
                if coverage:
                    ...
                    if testing_on_ci:
                        ...
                        run_command('codecov -f coverage.xml --root {}'.format(root))
                    else:
                        if not cov_keep:
