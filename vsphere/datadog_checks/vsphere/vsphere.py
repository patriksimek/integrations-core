...
class VSphereCheck(AgentCheck):
    ...
    *_atomic jobs perform one single task asynchronously in the ThreadPool, we
    ...
    def _get_cached_mors_items(self, instance_key):
        with self.morlist_lock:
            return self.morlist.get(instance_key, {}).items()

    def _get_cached_mors_values(self, instance_key):
        with self.morlist_lock:
    ...
    def get_external_host_tags(self):
        ...
        for instance in self.instances:
            i_key = self._instance_key(instance)
            mors = self._get_cached_mors_values(i_key)
            if not mors:
                self.log.warning(
        ...
        for resource_type in self.morlist_raw[i_key]:
            ...
            for _ in xrange(batch_size):
                try:
                    ...
                    cached_mors = self._get_cached_mors_values(i_key)
    ...
    def _vacuum_morlist(self, instance):
        ...
        """
        i_key = self._instance_key(instance)
        for mor_name, mor in self._get_cached_mors(i_key):
    ...
    def collect_metrics(self, instance):
        ...
        if i_key not in self.morlist:
            ...
            return
        mors = self._get_cached_mors_items(i_key)
        n_mors = len(mors)
